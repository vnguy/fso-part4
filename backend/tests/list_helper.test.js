...
test('totalLikes test 2', () => {
  const result = listHelper.totalLikes(single = [blogs[0]])
...
describe('favorite blog', () => {
  test('test favoriteBlog, return blog with most likes from a list of blogs', () => {
  ...
  test('new most liked author', () => {

    let newBlogs = blogs.map((i) => i)
    newBlogs.push({
    ...
    if(newBlogs.length !== blogs.length) {
      const results = listHelper.mostLikes(newBlogs)
